...
var TexParser = (function () {
    ...
    TexParser.prototype.GetDimen = function (name) {
        ...
        if (this.string.charAt(this.i) === '{') {
            ...
            var _a = __read(ParseUtil.matchDimen(dimen), 3), value = _a[0], unit = _a[1], _ = _a[2];
            if (value) {
                return value + unit;
            }
        }
            ...
            var _b = __read(ParseUtil.matchDimen(dimen, true), 3), value = _b[0], unit = _b[1], length_1 = _b[2];
            if (value) {
                this.i += length_1;
                return value + unit;
            }
    ...
    TexParser.prototype.GetDelimiterArg = function (name) {
        var c = ParseUtil.trimSpaces(this.GetArgument(name));
        if (c === '') {
            return null;
        }
        if (this.contains('delimiter', c)) {
