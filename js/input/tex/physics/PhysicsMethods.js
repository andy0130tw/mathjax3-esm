...
PhysicsMethods.Quantity = function (parser, name, open, close, arg, named, variant) {
    ...
    if (named) {
        var mml = parser.create('token', 'mi', { texClass: TEXCLASS.OP }, named);
        if (variant) {
...
PhysicsMethods.VectorBold = function (parser, name) {
    ...
    var node = new TexParser(arg, parser.stack.env, parser.configuration).mml();
    if (oldFont) {
...
var vectorApplication = function (parser, kind, name, operator, fences) {
    var op = new TexParser(operator, parser.stack.env, parser.configuration).mml();
...
PhysicsMethods.Expression = function (parser, name, opt, id) {
    ...
    if (exp) {
        var sup = new TexParser(exp, parser.stack.env, parser.configuration).mml();
...
PhysicsMethods.Differential = function (parser, name, op) {
    ...
    if (!(parens || braces)) {
        ...
        var mml = new TexParser(macro, parser.stack.env, parser.configuration).mml();
        parser.Push(mml);
        return;
    }
    if (braces) {
