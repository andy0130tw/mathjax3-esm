...
var paddedContent = function (parser, content) {
    var nodes = ParseUtil.internalMath(parser, ParseUtil.trimSpaces(content), 0);
...
BussproofsMethods.Label = function (parser, name, side) {
    ...
    var content = ParseUtil.internalMath(parser, parser.GetArgument(name), 0);
...
function parseFCenterLine(parser, name) {
    ...
    var premise = (new TexParser(prem, parser.stack.env, parser.configuration)).mml();
    var conclusion = (new TexParser(conc, parser.stack.env, parser.configuration)).mml();
    var fcenter = (new TexParser('\\fCenter', parser.stack.env, parser.configuration)).mml();
