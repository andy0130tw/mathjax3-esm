...
var getSpaces = function (inf, rule, right) {
    ...
    if (inf !== rule.parent) {
        ...
        if (NodeUtil.isType(children_1[index_1], 'mspace')) {
...
var adjustValue = function (inf, right) {
    ...
    var conc = getConclusion(rule, getProperty(rule, 'inferenceRule'));
...
var adjustSequents = function (config) {
    ...
    for (var i = sequents.length - 1, seq = void 0; seq = sequents[i]; i--) {
        ...
        while (getProperty(inf, 'inference') === 1) {
            inf = getRule(inf);
            var premise = firstPremise(getPremises(inf, getProperty(inf, 'inferenceRule')));
            var sequent = (getProperty(premise, 'inferenceRule')) ?
...
var addSequentSpace = function (config, sequent, position, direction, width) {
    var mspace = config.nodeFactory.create('node', 'mspace', [], { width: ParseUtil.Em(width) });
    if (direction === 'left') {
...
export var balanceRules = function (arg) {
    var e_1, _a;
    ...
    try {
        for (var inferences_1 = __values(inferences), inferences_1_1 = inferences_1.next(); !inferences_1_1.done; inferences_1_1 = inferences_1.next()) {
            ...
            var isProof = getProperty(inf, 'proof');
            var label = getProperty(inf, 'labelledRule');
            var rule = getRule(inf);
            var premises = getPremises(rule, getProperty(rule, 'inferenceRule'));
            ...
            var maxAdjust = getProperty(inf, 'maxAdjust');
            if (maxAdjust != null) {
...
var blacklistedProperties = (_a = {},
    ...
    _a);
export var setProperty = function (node, property, value) {
    NodeUtil.setProperty(node, property_prefix + property, value);
};
export var getProperty = function (node, property) {
    return NodeUtil.getProperty(node, property_prefix + property);
};
export var removeProperty = function (node, property) {
    node.removeProperty(property_prefix + property);
};
export var makeBsprAttributes = function (arg) {
