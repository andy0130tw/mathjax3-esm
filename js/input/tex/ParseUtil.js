...
(function (ParseUtil) {
    ...
    function fenced(configuration, open, mml, close, big) {
        ...
        var mrow = nf.create('node', 'mrow', [], { open: open, close: close, texClass: TEXCLASS.INNER });
        var mo;
        if (big) {
    ...
    function fixedFence(configuration, open, mml, close) {
        var mrow = configuration.nodeFactory.create('node', 'mrow', [], { open: open, close: close, texClass: TEXCLASS.ORD });
        if (open) {
        ...
        if (text.match(/\\?[${}\\]|\\\(|\\(eq)?ref\s*\{/)) {
            while (i < text.length) {
                ...
                if (c === '$') {
                    ...
                    if (match === '}' && braces === 0) {
                        var atom = (new TexParser(text.slice(k, i), {}, parser.configuration)).mml();
            ...
            try {
                for (var _b = __values(Object.keys(def)), _c = _b.next(); !_c.done; _c = _b.next()) {
                    ...
                    if (!allowed.hasOwnProperty(key)) {
                        if (error) {
                            throw new TexError('InvalidOption', 'Invalid optional argument: %1', key);
                        }
                        delete def[key];
                    }
                }
