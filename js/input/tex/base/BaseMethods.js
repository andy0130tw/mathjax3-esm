...
BaseMethods.Superscript = function (parser, c) {
    ...
    var movesupsub = NodeUtil.getProperty(base, 'movesupsub');
    var position = NodeUtil.isType(base, 'msubsup') ? base.sup :
        base.over;
...
BaseMethods.Subscript = function (parser, c) {
    ...
    var movesupsub = NodeUtil.getProperty(base, 'movesupsub');
    var position = NodeUtil.isType(base, 'msubsup') ?
        base.sub : base.under;
    if ((NodeUtil.isType(base, 'msubsup') && !NodeUtil.isType(base, 'msup') &&
...
BaseMethods.Middle = function (parser, name) {
    ...
    if (!parser.stack.Top().isKind('left')) {
        throw new TexError('MisplacedMiddle', '%1 must be within \\left and \\right', parser.currentCS);
    }
...
BaseMethods.NamedFn = function (parser, name, id) {
    ...
    var mml = parser.create('token', 'mi', { texClass: TEXCLASS.OP }, id);
...
BaseMethods.Sqrt = function (parser, name) {
    ...
    var mml = new TexParser(arg, parser.stack.env, parser.configuration).mml();
    if (!n) {
...
function parseRoot(parser, n) {
    ...
    env['inRoot'] = true;
    var newParser = new TexParser(n, env, parser.configuration);
...
BaseMethods.Accent = function (parser, name, accent, stretchy) {
    ...
    var def = ParseUtil.getFontDef(parser);
    def['accent'] = true;
    var entity = NodeUtil.createEntity(accent);
    var moNode = parser.create('token', 'mo', def, entity);
    var mml = moNode;
    NodeUtil.setAttribute(mml, 'stretchy', stretchy ? true : false);
    var mo = (NodeUtil.isEmbellished(c) ? NodeUtil.getCoreMO(c) : c);
...
BaseMethods.UnderOver = function (parser, name, c, stack, noaccent) {
    ...
    var symbol = NodeUtil.getForm(base);
    ...
    var entity = NodeUtil.createEntity(c);
...
BaseMethods.MmlToken = function (parser, name) {
    ...
    while (attr !== '') {
        ...
        var value = ParseUtil.MmlFilterAttribute(parser, match[1], match[2].replace(/^(['"])(.*)\1$/, '$2'));
        if (value) {
...
BaseMethods.Smash = function (parser, name) {
    var bt = ParseUtil.trimSpaces(parser.GetBrackets(name, ''));
...
BaseMethods.BuildRel = function (parser, name) {
    ...
    var atom = parser.create('node', 'TeXAtom', [node], { texClass: TEXCLASS.REL });
    parser.Push(atom);
};
...
BaseMethods.FBox = function (parser, name) {
    var internal = ParseUtil.internalMath(parser, parser.GetArgument(name));
...
BaseMethods.Dots = function (parser, name) {
    var ldotsEntity = NodeUtil.createEntity('2026');
    var cdotsEntity = NodeUtil.createEntity('22EF');
...
BaseMethods.Entry = function (parser, name) {
    ...
    if (parser.stack.Top().getProperty('isCases')) {
        ...
        if (!text.match(/^\s*\\text[^a-zA-Z]/) || close_1 !== text.replace(/\s+$/, '').length - 1) {
            var internal = ParseUtil.internalMath(parser, text, 0);
...
BaseMethods.CrLaTeX = function (parser, name, nobrackets) {
    ...
    if (!nobrackets && parser.string.charAt(parser.i) === '[') {
        ...
        var _a = __read(ParseUtil.matchDimen(dim), 3), value = _a[0], unit = _a[1], _ = _a[2];
        if (dim && !value) {
    ...
    if (top instanceof sitem.ArrayItem) {
        if (n && top.arraydef['rowspacing']) {
            ...
            if (!top.getProperty('rowspacing')) {
                var dimem = ParseUtil.dimen2em(rows[0]);
            ...
            rows[top.table.length - 1] = ParseUtil.Em(Math.max(0, rowspacing + ParseUtil.dimen2em(n)));
...
BaseMethods.HandleRef = function (parser, name, eqref) {
    ...
    var node = parser.create('node', 'mrow', ParseUtil.internalMath(parser, tag), {
