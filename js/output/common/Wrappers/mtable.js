...
export function CommonMtableMixin(Base) {
    return (function (_super) {
        ...
        function class_1() {
            ...
            _this.pwidthCells = [];
            _this.pWidth = 0;
            _this.numCols = max(_this.tableRows.map(function (row) { return row.numCells; }));
        ...
        class_1.prototype.getEqualColumns = function (width) {
            ...
            if (width === 'auto') {
                ...
                var w = sum([].concat(this.cLines, this.cSpace)) + 2 * this.fSpace[0];
        ...
        class_1.prototype.getColumnWidthsFixed = function (swidths, width) {
            ...
            var cwidth = width - sum([].concat(this.cLines, this.cSpace)) - 2 * this.fSpace[0];
            var dw = cwidth;
        ...
        class_1.prototype.getAlignmentRow = function () {
            var _a = __read(split(this.node.attributes.get('align')), 2), align = _a[0], row = _a[1];
            if (row == null)
                return [align, null];
            var i = parseInt(row);
            if (i < 0)
