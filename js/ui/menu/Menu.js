...
var Menu = (function () {
    function Menu(document, options) {
        ...
        this.about = new ContextMenu.Info('<b style="font-size:120%;">MathJax</b> v' + mathjax.version, function () {
            var lines = [];
        ...
        this.mathmlCode = new SelectableInfo('MathJax MathML Expression', function () {
            if (!_this.menu.mathItem)
                return '';
            var text = _this.toMML(_this.menu.mathItem);
            return '<pre>' + _this.formatSource(text) + '</pre>';
        }, '');
        this.originalText = new SelectableInfo('MathJax Original Source', function () {
            if (!_this.menu.mathItem)
                return '';
            var text = _this.menu.mathItem.math;
            return '<pre style="font-size:125%; margin:0">' + _this.formatSource(text) + '</pre>';
        }, '');
        this.annotationText = new SelectableInfo('MathJax Annotation Text', function () {
            if (!_this.menu.mathItem)
                return '';
    ...
    Menu.prototype.loadA11y = function (component) {
        ...
        this.loadComponent('a11y/' + component, function () {
            ...
            if (!Menu._loadingPromise) {
                _this.rerender(component === 'complexity' || noEnrich ? STATE.COMPILED : STATE.TYPESET);
            }
        });
    };
    ...
    Menu.prototype.rerender = function (start) {
        if (start === void 0) { start = STATE.TYPESET; }
